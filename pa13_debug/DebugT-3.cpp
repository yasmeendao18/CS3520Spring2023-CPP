...
template <class T>
CommaSeparatedList<T>::CommaSeparatedList(T *d, int s) //add * to d
{
   data = d;
   size = s;
}
template <class T>
    void CommaSeparatedList < T > ::printList() // chance < to >
{
   cout << "Comma separated list:" << endl; // change >> to <<
   for (int x = 0; x < size; ++x)
   {
      cout << data[x]; //change to x
      if (x != size - 1)
         cout << ", ";
   }
   cout << endl
...
ostream &operator<<(ostream &out, const Book &b)
{
   ...
   return out; // return out instead of ostream
}
...
int main()
{
   ...
   CommaSeparatedListSize = sizeof(someBook) / sizeof(someBook[0]); //change to sizeof
   ...
   CommaSeparatedListOfBooks.printList(); //change to printList
